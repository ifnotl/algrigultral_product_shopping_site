...
<head>
    ...
    $('#plotBtn').on('click', function() {
        var form_data = $('#searchForm').serialize() + '&format=json';
        $.ajax({
            type: 'GET',
            url: 'search_supply.php',
            data: form_data,
            success: function(response) {
                ...
                window.localStorage.setItem('chartData', JSON.stringify(data));
                window.open('plot_chart.php', '_blank');
            }
        });
    });
});
...
<body>
    ...
    <div class="search-container">
        <form id="searchForm" action="search_supply.php" method="GET">
            <input type="text" placeholder="产品名称" name="product_name" >
			...
			<button type="button" id="plotBtn">绘制价格对比图</button>
        </form>
    </div>
